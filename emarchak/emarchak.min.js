...
setTimeout(arguments.callee,200);};function $(id){return document.getElementById(id)};},switchAutoPage:function(){this.rF=!this.rF;var aE=document.getElementsByTagName('*');for(var i=0,l=aE.length;i<l;i++){if(aE[i].className=="tAP_switch"){aE[i].firstChild.nodeValue=this.rF?"AutoPage[OFF]":"AutoPage[ON]";}}}};window.addEventListener?window.addEventListener('load',tumblrAutoPager.init,false):window.attachEvent?window.attachEvent("onload",tumblrAutoPager.init):window.onload=tumblrAutoPager.init;jQuery(document).ready(function($){$('.pages li').each(function(){var link=$(this).find('a');var linkWidth=link.width();var iconWidth=link.find('i').width();var linkStyle=new Array();linkStyle['display']='block';linkStyle['width']=linkWidth;linkStyle['left']=iconWidth-linkWidth;link.css(linkStyle);});$('.pages li').hoverIntent(function(){var fullWidth=new Array();fullWidth['left']=0;$(this).find('a').animate(fullWidth,500);},function(){var link=$(this).find('a');var linkWidth=link.width();var iconWidth=link.find('i').width();var shortWidth=new Array();shortWidth['left']=iconWidth-linkWidth;link.animate(shortWidth,500);});});