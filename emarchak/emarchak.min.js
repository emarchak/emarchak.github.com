...
setTimeout(arguments.callee,200);};function $(id){return document.getElementById(id)};},switchAutoPage:function(){this.rF=!this.rF;var aE=document.getElementsByTagName('*');for(var i=0,l=aE.length;i<l;i++){if(aE[i].className=="tAP_switch"){aE[i].firstChild.nodeValue=this.rF?"AutoPage[OFF]":"AutoPage[ON]";}}}};window.addEventListener?window.addEventListener('load',tumblrAutoPager.init,false):window.attachEvent?window.attachEvent("onload",tumblrAutoPager.init):window.onload=tumblrAutoPager.init;jQuery(document).ready(function($){var window=$('html');var links=$('.menu li');function checkWidth(){var windowSize=window.width();if(windowSize>700){$(links).each(function(){var link=$(this).find('a');var linkWidth=link.width();var iconWidth=link.find('i').width();var linkStyle=new Array();linkStyle['display']='block';linkStyle['width']=linkWidth;linkStyle['left']=iconWidth-linkWidth;link.css(linkStyle);});}}
checkWidth();$(window).resize(checkWidth);links.hoverIntent(function(){$(this).find('a').animate({left:0},500);},function(){var link=$(this).find('a');var linkWidth=link.width();var iconWidth=link.find('i').width();var leftPos=iconWidth-linkWidth;link.animate({left:leftPos},500);});$('nav select').change(function(){var url=$(this).val();if(url){document.location.href=url;}
return false;});});